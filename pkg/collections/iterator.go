...
func (i *vecIterator[T]) size() int { //nolint:unused // false positive, used in product(...).
	return len(i.d)
}

func (i *vecIterator[T]) vecn() int { //nolint:unused // false positive, used in product(...).
	return i.n
}
...
// Vec1 wraps a slice of elements of type T with a vector iterator where
